...
from auth.sidebar_page import show_sidebar
from auth.analysis_form import show_analysis_form
from auth.footer import show_footer
from config.config import APP_NAME, APP_TAGLINE, APP_DESCRIPTION, APP_ICON

# Must be the first Streamlit command
st.set_page_config(
    page_title="HealthAI - Your Health Agent",
    page_icon="🩺",
    layout="wide"
)
...
def show_chat_history():
    success, messages = st.session_state.auth_service.get_session_messages(
        st.session_state.current_session['email']
    )
